...
def load_obs(field, objname, err_floor=0.05, zperr=True, **extras):
    ...
    newpath = '/home/jonathan/.conda/envs/snowflakes/lib/python2.7/site-packages/prospector/git/sfhphot_' + objname +\
              '_9_03_17_15_05'  # sfh_style
    print(newpath, 'path!!!')
    flux2 = []
    with open(newpath, 'r') as newp:
        for line in newp:
            if line[0] != '#':
                cols = line.split()
                for l in range(len(cols)):
                    if l == 0:
                        ...
                        flux2.append(float(cols[l]))
    # flux = np.squeeze(flux2)
    flux = np.array(flux2)
    flux_compare_obj = np.squeeze([dat[obj_idx]['f_' + f] for f in filternames])
    print('orig', flux_compare_obj)
    # flux = np.squeeze([newphot[obj_idx]['f_' + f] for f in filternames])
    print('flux', flux)
    # NEW: END differences for mass test

    # FOR NOW: just use uncertainties from the comparison object
    ...
    # NOTE: in input_sfh_test.py: photometry is recorded as uJy, NOT catalog fluxes, so conert back from uJy to maggies
    maggies = flux * 10**-6. / 3631
    maggies_unc = unc * 10**-6.44 / 3631  # CORRECTED (NOTE: unc still coming from catalog, at least for now)
...
model_params.append({'name': 'gas_logz', 'N': 1,
                     'isfree': True,  # DECOUPLE: True (False when coupled) (False for non-EELGs)
                     'init': 0.0,
...
model_params.append({'name': 'gas_logu', 'N': 1,
                     ...
                     'init': -1.0,  # BUCKET: NEW fix to -1.0
                     'units': '',
                     'prior_function': tophat,
...
def load_model(objname, field, agelims=[], **extras):
    ...
    # BUCKET: COMMENT [0] WHEN fracs.py, UNCOMMENT WHEN RUNNING prospector.py
...
RUNNING WITH
INSTEAD, RUN WITH:
mpirun -n 4 python prospector.py --param_file=eelg_simsfh_params.py
--outfile=out/out_simsfh/11063_cosmos_sim_9_03_17_15_05 --niter=2500 --field=cosmos --objname=11063

mpirun -n 4 python prospector.py --param_file=eelg_simsfh_params.py --outfile=out/out_simsfh/11063_sim_9_03_17_15_05
--niter=2500 --field=cosmos --objname=11063
'''
