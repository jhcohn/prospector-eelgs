...
model_params.append({'name': 'mass_units', 'N': 1,
                     'isfree': False,
                     'init': 'mstar'})  # mstar = stellar mass
# to convert SFH to SFR(t), need mass formed instead of mstar (requires calling fsps to convert)
...
def load_model(objname='', datname='', zname='', agelims=[], **extras):
    ...
    ncomp = len(agelims) - 1
    agelims = [0.0, 7.0, (7.0 + (np.log10(tuniv*1e9)-7.0)/4), (7.0 + 2*(np.log10(tuniv*1e9)-7.0)/4),
