...
def add_mask_sed(obj, field, ax):
    ...
    mask_base = '_out.pkl'
    mask_extra = mask_pre + 'extra' + mask_base  # includes SFH *AND* rest of extra_output, so call it extra and not sfh
    mask_res = mask_pre + 'res' + mask_base
    mask_sed = mask_pre + 'sed' + mask_base
    mask_restwave = mask_pre + 'restwave' + mask_base
    mask_spec = mask_pre + 'spec' + mask_base
    mask_spswave = mask_pre + 'spswave' + mask_base
    mask_chisq = mask_pre + 'chisq' + mask_base
    mask_justchi = mask_pre + 'justchi' + mask_base
    mask_files = [mask_extra, mask_res, mask_sed, mask_restwave, mask_spec, mask_spswave, mask_chisq, mask_justchi]
    print(mask_files)

    if os.path.exists(mask_files[4]):
        print('hooray')
        with open(mask_files[1], 'rb') as res:
            results = pickle.load(res)

        with open(mask_files[2], 'rb') as sed:
            sed = pickle.load(sed)

        with open(mask_files[3], 'rb') as restwave:
            wave_rest = pickle.load(restwave)

        with open(mask_files[4], 'rb') as spec:
            spec = pickle.load(spec)

        with open(mask_files[5], 'rb') as spswave:
        ...
        factor = 3631 * 10 ** 6 * (10 ** - 0.44)  # CORRECTION FACTOR
        res_jan = []
        err_jan = []
        sed_jan = []
        ...
        ax.plot(sps_wave, spec_jan, color='b', ls='--', alpha=0.5, label=r'Model Spectrum, Emission Masked')  # spectrum
        # ax.plot(wave_rest, sed_jan, 'D', color='k', markerfacecolor='None', markersize=10,
        #         markeredgewidth=1.25, markeredgecolor='k', label=r'Model Photometry')  # plot best fit model
        # ax.errorbar(wave_rest, res_jan, yerr=err_jan, marker='o', linestyle='', color='purple',
    ...
    for i in range(len(objs)):
        ...
        if os.path.exists(files[1]):
            ...
            if fields[i] == 'cosmos':
                ...
                place = uds_04
                pre = uds_pre
                main = '/home/jonathan/uds/uds.v1.5.10.cat'
            ...
            if os.path.exists(fast_file):
                ...
                masked_spec = add_mask_sed(obj, field, ax1)

                # ax1.text(1000, 50, 'EELG', fontsize=20)
                # ax1.text(textx, texty, 'COSMOS-1824 (EELG)', fontsize=20)  # 700, 600
                ax1.text(textx, 20, field.upper() + '-' + str(obj), fontsize=20)  # 700, 600
                # ax1.text(textx, 6, 'M$_{\rm FAST}$ / M$_{\rm Prosp}$' + str((10 ** float(fast_mass)) /
                #                                                             (10 ** float(prosp_mass))), fontsize=20)  # 700, 600
                ...
                if masked_spec != 0:
                    ax2 = plt.subplot(gs[1], sharex=ax1)  # smaller lower axis
                    delt = []
                    for sp in range(len(masked_spec)):
                        delt.append(masked_spec[sp] / spec_jan[sp])
                    ax2.plot(sps_wave, delt, color='k', label='Masked spectrum / Regular spectrum')
                    ax2.legend(numpoints=1, loc='upper left', prop={'size': 20})
                    print(np.median(delt), max(delt), min(delt))
                    ax2.set_ylim(0.1, 3.4)  # (0.4, 1.9)
                    ax2.set_yticks([0.5, 1., 1.5, 2.0, 2.5, 3.0])
