...
def load_obs(field, objname, err_floor=0.05, zperr=True, **extras):
    ...
    for i in range(len(range_rest)):
        ...
        # if left edge of filter range is < 1216, OR if left edge is < 1180 and right edge > 1180
        # OR if left edge is < 1260 and right edge is > 1260

    # NEW, ALSO MASK POINTS AROUND [OIII]+HBeta
    oh_mask = []  # 4861 to 5007
    for i in range(len(range_rest)):
        oh_mask.append(((range_rest[i][0] < 4861) & (range_rest[i][1] > 4861))
                       or ((range_rest[i][0] < 5007) & (range_rest[i][1] > 5007))
                       or ((range_rest[i][0] > 4861) & (range_rest[i][1] < 5007)))
        # if left edge is < 4861 and right edge is > 4861; OR if left edge is < 5007 and right edge > 5007
        # OR if left edge is > 4861 and right edge is < 5007
    # print('mask', oh_mask)  # mask out ly_alpha and all points blueward of 1216
