...
if __name__ == "__main__":
    ...
    for file in os.listdir(eoute):
        ...
        if file.endswith(".h5") and float(key) < 200:
            egals[key] = file
            ekeys.append(key)
    # NOTE: for eetest2 these three all had len(12, 6[, 10**3])
    ...
    for i in range(len(egals)):  # (12): # for each galaxy (0 through 99)
        # i = allkeys[ind]
        ...
        pars = git + 'eetest/sfhtest_' + str(i) + '_params.py'  # eetest2
        with open(pars, 'r') as parfile:
            for line in parfile:
                if line.startswith("# Codename: "):
                    spaces = 0
