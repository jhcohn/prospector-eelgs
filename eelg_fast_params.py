...
def get_names(field):
    if field == 'cosmos':
        ...
        zname = '/scratch/user/joncohn/cosmos.v1.3.6.awk.zout'
        fname = '/scratch/user/joncohn/cosmos.v1.3.6.awk.fout'
        ...
        fname = '/scratch/user/joncohn/cdfs.v1.6.9.awk.fout'
        ...
        fname = '/scratch/user/joncohn/uds.v1.5.8.awk.fout'
...
def load_obs(field, objname, err_floor=0.05, zperr=True, **extras):
    ...
    # print(phot_mask)
    # 10**-6
    maggies = flux * 10**-6.44 / 3631  # flux [uJy] * 1e-6 [Jy / uJy] * 1 [maggy] / 3631 [Jy]
    maggies_unc = unc * 10**-6.44 / 3631
...
model_params.append({'name': 'logzsol', 'N': 1,
                     'isfree': False,  # True,  # fixedmet
                     'init': -0.7,  # -1.0,  # fixedmet (fast fixed to 1/5 solar)
                     'init_disp': 0.4,
                     'log_param': True,
...
model_params.append({'name': 'sfh', 'N': 1,
                     'isfree': False,
                     'init': 4,  # NEW: 0 = non-parametric SFH; 1 = parametric SFH, 4 = delayed tau SFH
                     'units': 'type',
...
model_params.append({'name': 'tau', 'N': 1,
                     ...
                     'units': 'Gyr',
                     'prior_function': tophat,
                     'prior_args': {'mini': 0.01, 'maxi': 100.0}})  # 0.03

model_params.append({'name': 'logtau', 'N': 1,
                     'isfree': True,  # NEW turn off
                     'init': 1,
                     'init_disp': 0.5,
                     'units': 'Gyr',
                     'prior_function': tophat,
                     'prior_args': {'mini': -2.0, 'maxi': 2.0}}) # -1.52
...
model_params.append({'name': 'tage', 'N': 1,
			...
			'units': 'Gyr',
			'prior_function': tophat,
			'prior_args': {'mini': 0.03, 'maxi': 12.6}})  # 0.01, 14.0  (fast file runs age from 10^7.5 to 10^10.1 (in yr)
...
model_params.append({'name': 'dust2', 'N': 1,
                     ...
                     'units': '',
                     'prior_function': tophat,
                     'prior_args': {'mini': 0.0, 'maxi': 4.0 / 1.086}})
...
model_params.append({'name': 'gas_logz', 'N': 1,
                     ...
                     'init': 0.0,
                     'depends_on': tie_gas_logz,  # not specified in Salmon+2015, so fix to stellar
                     'units': r'log Z/Z_\odot',
                     'prior_function': tophat,
...
model_params.append({'name': 'add_neb_continuum', 'N': 1,  # BUCKET1
                     'isfree': False,
                     'init': False,  # specified false in Forrest17
                     'units': '',
                     'prior_function_name': None,
                     'prior_args': None})
...
model_type = sedmodel.SedModel  # BurstyModel

'''
RUNNING WITH
mpirun -n 4 python prospector.py --param_file=eelg_fast_params.py --outfile=12105_cosmos_fast --niter=2500 --field=cosmos --objname=12105
'''
